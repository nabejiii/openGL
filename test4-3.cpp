...
void glut_display(){
	...
	GLfloat lightpos1[] = {5 * cosf((float)g_angle3) * sinf((float)g_angle4), 
		5 * sinf((float)g_angle3), 
		5 * cosf((float)g_angle3) * cosf((float)g_angle4), 
		1.0f};
	...
	glPushMatrix();
	glTranslatef(5 * cosf((float)g_angle3) * sinf((float)g_angle4),
		5 * sinf((float)g_angle3),
		5 * cosf((float)g_angle3) * cosf((float)g_angle4));
	glutSolidSphere(0.2, 50, 50);
	glPopMatrix();
...
void draw_pyramid(){
	...
	glNormal3d(0.0, 0.6, 0.8); // (0.0, 0.6, 0.0)にしたら全然反射しなくなった
