...
int g_TextureCount1 = 0;
int g_TextureCount2 = 1;
...
void init(){
	...
	for(int i = 0; i < 3; i++){
        ...
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); // GL_NEARESTは近傍補間
...
void draw_pyramid(){
	...
	glBindTexture(GL_TEXTURE_2D, g_TextureHandles[g_TextureCount1]);

	glBegin(GL_TRIANGLES);
	glVertex3dv(pointO); glTexCoord2d(0,0);
	glVertex3dv(pointD); glTexCoord2d(1,0);
	glVertex3dv(pointA); glTexCoord2d(0.5,1);
	glEnd();


	glColor3d(1.0, 1.0, 1.0);

	glBindTexture(GL_TEXTURE_2D, g_TextureHandles[g_TextureCount2]);
